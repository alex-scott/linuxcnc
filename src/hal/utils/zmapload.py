...
def readpoints(filename, zlimit):
    ...
    if abs(zmin) > zlimit:
        print("z map height=%g > %g limit exceeded, abort!" % (zmin, zlimit), file=sys.stderr)
        return None

    print("z map height=%g" % (zmin), file=sys.stderr)
...
def make_grid(points):
    ...
    if nx <= 0 or ny <= 0:
        print("need at lead one point!", file=stderr)
        return False
    if nx > 32 or ny > 32:
        print("max grid is 32x32!", file=stderr)
        return False
